...
def add_payment_period(start_date, end_date, lecture_rate, practice_rate, seminar_rate):
    connection = pyodbc.connect('DRIVER={SQL Server};SERVER=DESKTOP-1OJ7R9S\\SQLEXPRESS;DATABASE=test')
    cursor = connection.cursor()
    cursor.execute('exec Add_payment_period @start_date=' + start_date + ', @end_date=' + end_date +
                   ', @lecture_payment_rate=' + lecture_rate + ',@practice_payment_rate=' + practice_rate
                   + ', @seminar_payment_rate=' + seminar_rate)
    connection.close()
