...
def import_data():
    ...
    while True:
        ...
        if lecturer is None:
            break
        lecturers.append(model.Lecturer(lecturer[0], lecturer.email, lecturer.name, lecturer.surname, lecturer.patronymic))
    ...
    while True:
            ...
            if study_plan is None:
                break
            study_plans.append(model.StudyPlan(study_plan.year, study_plan.program, next(stream for stream in streams if stream.id == study-plan.id)))
    ...
    while True:
           ...
           if payment_period is None:
               break
           payment_periods.append(model.PaymentPeriod(payment_period.id, payment_period.start_date, payment_period.end_date, payment_period.lecture_payment_rate,
                                                      payment_period.practice_session_payment_rate, payment_period.seminar_payment_rate))
